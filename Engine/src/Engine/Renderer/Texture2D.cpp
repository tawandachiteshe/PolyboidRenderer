...
namespace Polyboid
{
	Texture::Texture(int32_t width, int32_t height, const TextureSettings& settings): m_Width(width), m_Height(height),
		m_InternalFormat(0)
	{
	...
	Texture::Texture(int32_t width, int32_t height, int32_t channels): m_Width(width), m_Height(height),
	                                                                   m_Channels(channels)
	{
		POLYBOID_PROFILE_FUNCTION();
	...
	void Texture::Bind(uint32_t slot, bool computeTexture)
	{
		...
		if (computeTexture)
		{
			glBindImageTexture(slot, m_TextureID, 0, GL_FALSE, 0, GL_READ_WRITE, m_InternalFormat);
		}
		else
		{
